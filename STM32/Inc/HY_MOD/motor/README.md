...
│   ├── ref_ori (MotorCtrl)
│   └── ref_fix (MotorCtrl)
├── rotate_h (MotorRotParameter) // 馬達旋轉模式
│   ├── ref_ori (MotorRot)
│   └── ref_fix (MotorRot)
├── speed_h (MotorSpdParameter) // 從尾往轉子 順時針value為負
│   ├── ref_rpm (float32_t)
│   ├── ref_omega (float32_t)
│   ├── fbk_rpm (float32_t)
│   ├── fbk_omega (float32_t)
...
├── adc_h (MotorADC) // 電流 ADC  <====== (依最新程式碼已移至此處)
...
│   ├── reverse (bool)
│   ├── duty_val (float32_t) // DEG duty
│   ├── duty_h (MotorPhaseDuty) // DEG duty
│   │   ├── u (float32_t)
│   │   ├── v (float32_t)
│   │   └── w (float32_t)
│   ├── pi_omega (PI_CTRL)
